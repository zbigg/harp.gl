...
import { Color, ImageDecoder, ImageDecoderConstructor } from "./ImageFactory";
// tslint:disable-next-line: no-duplicate-imports
import { ImageEncoder, ImageEncoderConstructor } from "./ImageFactory";
...
export class ImageVectorDecoderConstructor implements ImageDecoderConstructor {
    ...
    load(filePath: string): Promise<ImageDecoder> {
        return new Promise((resolve, reject) => {
            const loadPromise = FileSystem.readFile(filePath);
            loadPromise
                .then(
                    (fileBuffer: Buffer) => {
                        svg2png(fileBuffer,
                            ...
                            .then((pngBuffer: Buffer) => {
                                Jimp.read(pngBuffer)
                                    .then((bitmap: Jimp) => {
                                        resolve(new ImageVectorDecoder(fileBuffer, bitmap));
                                    });
                            });
                    }
                )
...
export class ImageVectorEncoderConstructor implements ImageEncoderConstructor {
    ...
    create(width: number, height: number): Promise<ImageEncoder> {
        ...
        const bitmapEncoderCtor: ImageBitmapEncoderConstructor =
            new ImageBitmapEncoderConstructor();
...
class ImageVectorEncoder implements ImageEncoder {
    ...
    getPixelAt(x: number, y: number): Color {
        if (x < this.width && y < this.height) {
            ...
            throw new Error("Access to pixel outside the declared boundaries," +
        ...
        if (x < this.width && y < this.height) {
            try {
                this.m_bitmap.setPixelColor(
                    Jimp.rgbaToInt(color.r, color.g, color.b, color.a), x, y,
    ...
    write(filePath: string): Promise<any> {
        if (!this.m_pixelModified &&
            FileSystem.getImageFormat(filePath) === ImageFormat.SVG) {
    ...
    writeSync(filePath: string): void {
        if (!this.m_pixelModified &&
            ...
            if (!checkExportPath(filePath)) {
                throw new Error("Failed to write and image: " + filePath +
                    " only BMP, PNG, JPG formats are supported");
            }
            const mime = FileSystem.getImageMimeType(filePath);
            this.m_bitmap.getBuffer(mime, (err, buffer) => {
                if (err) {
                    throw new Error("Could not get buffer to write and image: " + filePath);
                }
...
function getJimpPixelAt(image: Jimp.Jimp, x: number, y: number): Color {
    ...
    const rgba: Color = {
        r: ColorUtils.alpha(value),
        g: ColorUtils.blue(value),
        b: ColorUtils.green(value),
        a: ColorUtils.red(value)
    };
    return rgba;
}
