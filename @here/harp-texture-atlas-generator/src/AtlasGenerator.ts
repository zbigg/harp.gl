...
import { BlendAlpha, BlendAlphaPremultiplied, BlendMultiply } from "./BlendOperations";
// tslint:disable-next-line: no-duplicate-imports
import { BlendMultiplyRGB, BlendOperation, BlendOverlay, BlendScreen } from "./BlendOperations";
...
import { AddForeground, BlendImages, Colorize, CombineImages, Grayscale } from "./ImageProcessing";
// tslint:disable-next-line: no-duplicate-imports
import { AddBackground, GrayscaleMethod, ImageProcess } from "./ImageProcessing";
// tslint:disable-next-line: no-duplicate-imports
import { InvertColor, Resize } from "./ImageProcessing";
import { ReferenceImg } from "./ImageUtils";
...
export async function generateSprites(options: ProcessingOptions): Promise<string[]> {
    ...
    const outputFiles: string[] = await prepareSprites(inputFiles, options.processConfig,
        ImageFormat.PNG, outputDir, options.width, options.height, logger, options.jobs);
...
export async function generateSpritesAtlas(options: AtlasOptions): Promise<any> {
    ...
    try {
        // Post-process and prepare input files for atlas creation, export them to PNG format.
        const intermediateFiles: string[] = await prepareSprites(inputFiles, options.processConfig,
            ImageFormat.PNG, tempOutputDir, options.width, options.height, logger, options.jobs);

        // Finally generate atlas from it.
        const result = await mergeSpritesIntoAtlas(intermediateFiles, outputPath, options.padding,
            options.minify, logger);
...
async function prepareSprites(inputFiles: string[], processConfigPath: string,
    outputFormat: ImageFormat, outputDir: string, outWidth: number, outHeight: number,
    logger: Logger, jobsNum: number = 8): Promise<string[]> {
    ...
    if (processConfigPath !== undefined && processConfigPath.length > 0) {
        ...
        const processingConfig: any = JSON.parse(
            FileSystem.readFileSync(configPath).toString());
    ...
    const images: ImageEncoder[] = await processImages(inputFiles, imageFactory,
        processingSteps, logger, jobsNum);

    // Store all post-processed images into output directory.
    return storeImages(inputFiles, images, outputDir, outputFormat);
}

function processImages(inputFiles: string[], imageFactory: ImageFactory,
    operations: ImageProcess[], logger: Logger, jobsNum: number = 8,
    outputDir?: string): Promise<ImageEncoder[]> {
...
async function storeImages(inputFiles: string[], images: ImageEncoder[],
    outputDir: string, outputFormat: ImageFormat = ImageFormat.PNG): Promise<string[]> {
...
function mergeSpritesIntoAtlas(inputFiles: string[], outputPath: string,
    paddingSize: number, minify: boolean, logger: Logger): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
        spritesmith.run({ src: inputFiles, padding: paddingSize },
...
async function readAndProcessImage(inFile: string, imageFactory: ImageFactory,
    operations: ImageProcess[], logger: Logger,
    outFile?: string): Promise<ImageEncoder> {
    ...
    switch (processName) {
        ...
        case AddForeground.name:
            if (jsonElement.image === undefined) {
                throw new Error(`${processName} 'image' attribute ` +
                    `is not defined for processing step!`);
            }
            else {
                const offset = jsonElement.offset !== undefined ?
                    jsonElement.offset : { x: 0, y: 0 };
        ...
        case CombineImages.name:
            if (jsonElement.image === undefined) {
                throw new Error(`Destination 'image' attribute not defined ` +
                    `for ${processName} step!`);
            }
            else if (jsonElement.blendMode === undefined) {
                throw new Error(`Atrribute 'blendMode' is not defined ` +
                    `for ${processName} step!`);
            }
            ...
            if (blendOperation === undefined) {
                throw new Error(`Atrribute 'blendMode' not recognized as proper BlendOperation ` +
                    `for ${processName} step!`);
            }
            ...
            if (jsonElement.name === BlendImages.name) {
                ...
                const offset = jsonElement.offset !== undefined ?
                    jsonElement.offset : { x: 0, y: 0 };
                const sizeRef: ReferenceImg = jsonElement.sizeRef !== undefined ?
                    jsonElement.sizeRef : ReferenceImg.Bigger;
