...
program
    ...
    .option("-w, --width [num]", "Width of sprite in atlas, set to zero leaves size unchanged " +
        "or constraints to height (if set) while preserving aspect ratio", _defaultWidth)
    .option("-h, --height [num]", "Height of sprite in atlas, zero leaves original size " +
        "or constraints to width (if set) while preserving aspect ratio", _defaultHeight)
...
if (options.verbose) {
    ...
    logger.log(LogLevel.DEBUG, "Sprite width: %s", options.width);
    logger.log(LogLevel.DEBUG, "Sprite height: %s", options.height);
    logger.log(LogLevel.DEBUG, "Padding: %s", options.padding);
