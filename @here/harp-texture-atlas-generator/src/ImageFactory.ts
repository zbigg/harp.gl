...
// TODO: Figure out some better pattern - static vs instance interface
// maybe use prototype trick.
export interface ImageDecoderConstructor {
    //readonly prototype: ImageDecoder;
    //new(filePath: string): ImageDecoder;

    load(filePath: string): Promise<ImageDecoder>;
}

export interface ImageEncoderConstructor {
    //readonly prototype: ImageEncoder;
    //new(width: number, height: number): ImageEncoder;

    create(width: number, height: number): Promise<ImageEncoder>;
}

//declare const ImageDecoder: ImageDecoderConstructor;
//declare const ImageEncoder: ImageEncoderConstructor;
...
export class ImageFactory {
    ...
    registerImageType(format: ImageFormat, decoderC: ImageDecoderConstructor,
        encoderC: ImageEncoderConstructor): void {
